...
void Geometry(point Varyings input[1], inout TriangleStream<VertOut> outStream)
{
	...
	float2 extent = abs(UNITY_MATRIX_P._11_22 * size * _PointSize * _PointSizeScale);
...
FragOut Fragment(VertOut input)
{
	...
	if (_UseInterpolation > 0.5) {
		...
		input.viewposition.z += (1 - uvlen) * input.size * _PointSize * _PointSizeScale * 2.0f;
	}
	
	
	float4 pos = mul(UNITY_MATRIX_P, input.viewposition);
	pos /= pos.w;
	//half4 clr = (( (pos.x / 2.0 + 0.5 - _EyeGaze.x) * (pos.x / 2.0 + 0.5 - _EyeGaze.x) + (pos.y / 2.0 + 0.5 - _EyeGaze.y) * (pos.y / 2.0 + 0.5 - _EyeGaze.y) ) < 0.0001 ? _Highlight : c);
